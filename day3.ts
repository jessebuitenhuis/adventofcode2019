...
class Circuit {
  ...
  constructor(data: string) {
    this.wires = data.split("\n").map(x => new Wire(x));
  }

  get intersections(): Coordinate[] {
    const segments1 = this.wires[1].segments;
    const segments2 = this.wires[0].segments;

    return segments1
      .map(a => {
        return segments2
          .map(b => a.findIntersectionWith(b))
          .filter(x => !!x) as Coordinate[];
      })
      .flat();
  }

  getDistanceToCoord(coord: Coordinate): number {
    return this.wires.reduce(
      (total, wire) => total + wire.getStepsToCoord(coord),
      0
    );
  }
}
...
class Segment {
  direction: Direction = this._raw[0] as Direction;
  steps: number = parseInt(this._raw.slice(1));

  start: Coordinate = this.getStart();
  // isHorizontal: boolean = this.getIsHorizontal();
  end: Coordinate = this.getEnd();
  ...
  constructor(public _raw: string, public previous?: Segment) {}

  getStart(): Coordinate {
    return (this.previous && this.previous.end) || new Coordinate(0, 0);
  }
  ...
  getEnd(): Coordinate {
    return this.start.move(this.direction, this.steps);
  }
  ...
  get maxY(): number {
    return Math.max(this.start.y, this.start.y);
  }

  intersectsWith(segment: Segment): boolean {
    if (
      (this.maxX < segment.minX && this.minX > segment.maxX) ||
      (this.maxY < segment.minY && this.minY > segment.maxY)
    ) {
      return false;
    }
  ...
  findIntersectionWith(segment: Segment): Coordinate | undefined {
    for (let x = this.minX; x <= this.maxX; x++) {
      for (let y = this.minY; y <= this.maxY; y++) {
        const coord = new Coordinate(x, y);
        if (coord.isOnSegment(segment)) {
          return coord;
        }
      }
    }
...
class Wire {
  segments: Segment[] = this._getSegments();

  constructor(private _data: string) {}

  getStepsToCoord(coord: Coordinate): number {
    let steps = 0;

    for (var i = 0; i < this.segments.length; i++) {
      const segment = this.segments[i];
      if (coord.isOnSegment(segment)) {
        const partial = coord.getDistanceFrom(segment.start.x, segment.start.y);
        steps += partial;
        console.log("Intersection found", coord, segment.start, partial);

        break;
      } else {
        steps += segment.steps;
        // console.log("Steps after segment", segment._raw, segment.steps, steps);
      }
    }
  ...
  private _getSegments() {
    return this._data
      .split(",")
      .reduce(
        (list, x) => [...list, new Segment(x, list[list.length - 1])],
        [] as Segment[]
      );
  }
}
...
class Coordinate {
  ...
  move(direction: Direction, steps: number): Coordinate {
    switch (direction) {
      case "L":
...
const circuit = new Circuit(raw);
const intersections = circuit.intersections;
const distances = intersections.map(x => x.distance).sort((a, b) => a - b);
const combinedDistances = intersections
  .map(x => circuit.getDistanceToCoord(x))
  .sort((a, b) => a - b);
console.log("Distances", combinedDistances);

// console.log(circuit.wires[0].segments.map(x => x.end));
