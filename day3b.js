...
const input3 = raw.split("\n").map(line => line.split(","));

const coordinates = input3.map(line => {
  // Segment: [startX, startY, endX, endY];
  // start should always be the lowest
  const coord = [0, 0];

  return line.map(instruction => {
...
const intersections = coordinates
  .map(line => {
    const otherLines = coordinates.filter(x => x !== line);
    const otherSegments = [].concat(...otherLines);
    ...
    return line
      .map(segment => {
        ...
        for (let i = 0; i < otherSegments.length; i++) {
          intersection = linesIntersect(otherSegments[i], segment);
          if (intersection) {
            break;
          }
        }
...
const distances = intersections
  .map(intersection => {
    const i = intersection.split(",");
    return Math.abs(parseInt(i[0])) + Math.abs(parseInt(i[1]));
  })
  .sort((a, b) => a - b);

// console.log(distances);

function getStepsForLine(line, intersection) {
  let steps = 0;
  console.log("finding intersection", intersection);
  const splitted = intersection.split(",");
  const intersectionX = parseInt(splitted[0]);
  const intersectionY = parseInt(splitted[1]);

  // loop through instructions till we find the intersection
  for (let i = 0; i < line.length; i++) {
    const coordinate = line[i];

    const isHorizontal = coordinate[2] === coordinate[0];

    // Iterate x when horizontal, else y
    const startPoint = isHorizontal ? coordinate[0] : coordinate[1];
    const endPoint = isHorizontal ? coordinate[2] : coordinate[3];

    for (let j = startPoint; j <= endPoint; j++) {
      steps++;

      const x = isHorizontal ? j : coordinate[0];
      const y = isHorizontal ? coordinate[1] : j;

      if (x === intersectionX && y === intersectionY) {
        console.log("here", x, intersectionX, y, intersectionY);
        return steps;
      }
    }
  }
...
const steps = intersections
  .map(intersection => {
    return coordinates.reduce((totalSteps, line) => {
      totalSteps += getStepsForLine(line, intersection);
      return totalSteps;
    }, 0);
  })
  .sort((a, b) => b - a);
...
function linesIntersect(segment1, segment2) {
  [startX1, startY1, endX1, endY1] = segment1;
  [startX2, startY2, endX2, endY2] = segment2;

  if (
    (startX1 < startX2 && endX1 < startX2) ||
    (startX1 > endX2 && endX1 > endX2) ||
    (startY1 < startY2 && endY1 < startY2) ||
    (startY1 > endY2 && endY1 > endY2)
  ) {
    return null;
  }

  const x = startX1 === endX1 ? startX1 : startX2;
  const y = startY1 === endY1 ? startY1 : startY2;
